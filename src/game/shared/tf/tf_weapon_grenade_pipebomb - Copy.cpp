	...
	if ( m_iType == TF_GL_MODE_REMOTE_DETONATE )
	{
		...
		else
		{
			...
			else
			{
				...
				{
					return "stickybombtrail_ylw";
				}
				else
				{
					if (GetTeamNumber() == TF_TEAM_GRN)
					{
						return "stickybombtrail_grn";
					}
					else
					{
						if (GetTeamNumber() == TF_TEAM_PRP)
						{
							return "stickybombtrail_prp";
						}
						else
						{
							return "stickybombtrail_brw";
						}
					}
				}
			}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				if (GetTeamNumber() == TF_TEAM_YLW)
				{
					return "pipebombtrail_ylw";
				}
				else
				{
					if (GetTeamNumber() == TF_TEAM_GRN)
					{
						return "pipebombtrail_grn";
					}
					else
					{
						if (GetTeamNumber() == TF_TEAM_PRP)
						{
							return "pipebombtrail_prp";
						}
						else
						{
							return "pipebombtrail_brw";
						}
				...
				if ((gpGlobals->curtime - m_flCreationTime) >= tf_grenadelauncher_livetime.GetFloat())
				{
					...
					else
					{
						...
						else
						{
							if (GetTeamNumber() == TF_TEAM_YLW)
							{
								ParticleProp()->Create("stickybomb_pulse_ylw", PATTACH_ABSORIGIN);
							}
							else
							{
								if (GetTeamNumber() == TF_TEAM_GRN)
								{
									ParticleProp()->Create("stickybomb_pulse_grn", PATTACH_ABSORIGIN);
								}
								else
								{
									if (GetTeamNumber() == TF_TEAM_PRP)
									{
										ParticleProp()->Create("stickybomb_pulse_prp", PATTACH_ABSORIGIN);
									}
									else
									{
										ParticleProp()->Create("stickybomb_pulse_brw", PATTACH_ABSORIGIN);
									}
								}
							}
						}
