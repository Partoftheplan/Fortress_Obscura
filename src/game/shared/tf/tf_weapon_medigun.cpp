	...
	if ( GetTFPlayerOwner() && bOwnerTaunting == false && m_bHolstered == false && ( m_flChargeLevel >= 1.0f || m_bChargeRelease == true ) )
	{
		if ( m_pChargeEffect == NULL )
		{
			...
			case TF_TEAM_RED:
				...
				break;
			case TF_TEAM_YLW:
				pszEffectName = "medicgun_invulnstatus_fullcharge_ylw";
				break;
			case TF_TEAM_GRN:
				pszEffectName = "medicgun_invulnstatus_fullcharge_grn";
				break;
			case TF_TEAM_PRP:
				pszEffectName = "medicgun_invulnstatus_fullcharge_prp";
				break;
			case TF_TEAM_BRW:
				pszEffectName = "medicgun_invulnstatus_fullcharge_brw";
				break;
			default:
				pszEffectName = "";
				break;
...
void CWeaponMedigun::UpdateEffects( void )
{
	...
	if ( m_hHealingTarget )
	{
		...
		{
			if (pFiringPlayer->GetTeamNumber() == TF_TEAM_BLUE)
			{
				if (m_bChargeRelease)
				{
			...
			{
				if (pFiringPlayer->GetTeamNumber() == TF_TEAM_YLW)
				{
					if (m_bChargeRelease)
					{
						pszEffectName = "medicgun_beam_ylw_invun";
					}
					else
					{
						pszEffectName = "medicgun_beam_ylw";
					}
				}
				else
				{
					if (pFiringPlayer->GetTeamNumber() == TF_TEAM_GRN)
					{
						if (m_bChargeRelease)
						{
							pszEffectName = "medicgun_beam_grn_invun";
						}
						else
						{
							pszEffectName = "medicgun_beam_grn";
						}
					}
					else
					{
						if (pFiringPlayer->GetTeamNumber() == TF_TEAM_PRP)
						{
							if (m_bChargeRelease)
							{
								pszEffectName = "medicgun_beam_prp_invun";
							}
							else
							{
								pszEffectName = "medicgun_beam_prp";
							}
						}
						else
						{
							if (m_bChargeRelease)
							{
								pszEffectName = "medicgun_beam_brw_invun";
							}
							else
							{
								pszEffectName = "medicgun_beam_brw";
							}
						}
					}
				}
