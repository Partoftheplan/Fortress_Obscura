...
void CTFTeamMenu::ShowPanel( bool bShow )
{
	...
	if ( bShow )
	{
		...
		case TF_TEAM_YLW:
			if (IsConsole())
			{
				m_pYlwTeamButton->OnCursorEntered();
				m_pYlwTeamButton->SetDefaultAnimation("enter_enabled");
			}
			GetFocusNavGroup().SetCurrentFocus(m_pYlwTeamButton->GetVPanel(), m_pYlwTeamButton->GetVPanel());
			break;

		case TF_TEAM_GRN:
			if (IsConsole())
			{
				m_pGrnTeamButton->OnCursorEntered();
				m_pGrnTeamButton->SetDefaultAnimation("enter_enabled");
			}
			GetFocusNavGroup().SetCurrentFocus(m_pGrnTeamButton->GetVPanel(), m_pGrnTeamButton->GetVPanel());
			break;

		case TF_TEAM_PRP:
			if (IsConsole())
			{
				m_pPrpTeamButton->OnCursorEntered();
				m_pPrpTeamButton->SetDefaultAnimation("enter_enabled");
			}
			GetFocusNavGroup().SetCurrentFocus(m_pPrpTeamButton->GetVPanel(), m_pPrpTeamButton->GetVPanel());
			break;

		case TF_TEAM_BRW:
			if (IsConsole())
			{
				m_pBrwTeamButton->OnCursorEntered();
				m_pBrwTeamButton->SetDefaultAnimation("enter_enabled");
			}
			GetFocusNavGroup().SetCurrentFocus(m_pBrwTeamButton->GetVPanel(), m_pBrwTeamButton->GetVPanel());
			break;
...
void CTFTeamMenu::OnCommand( const char *command )
{
	...
	if ( Q_stricmp( command, "vguicancel" ) )
	{
		...
		if ( pLocalPlayer && ( Q_strstr( command, "jointeam " ) ) )
		{
			...
			else if (Q_stricmp(pTeam, "ylw") == 0)
			{
				iTeam = TF_TEAM_YLW;
			}
			else if (Q_stricmp(pTeam, "grn") == 0)
			{
				iTeam = TF_TEAM_GRN;
			}
			else if (Q_stricmp(pTeam, "prp") == 0)
			{
				iTeam = TF_TEAM_PRP;
			}
			else if (Q_stricmp(pTeam, "brw") == 0)
			{
				iTeam = TF_TEAM_BRW;
			}
			...
			if (iTeam == TF_TEAM_YLW && m_bYlwDisabled)
			{
				return;
			}

			if (iTeam == TF_TEAM_GRN && m_bGrnDisabled)
			{
				return;
			}

			if (iTeam == TF_TEAM_PRP && m_bPrpDisabled)
			{
				return;
			}

			if (iTeam == TF_TEAM_BRW && m_bBrwDisabled)
			{
				return;
			}
...
void CTFTeamMenu::OnTick()
{
	...
	C_Team *pYlw = GetGlobalTeam( TF_TEAM_YLW );
	C_Team *pGrn = GetGlobalTeam( TF_TEAM_GRN );
	C_Team *pPrp = GetGlobalTeam( TF_TEAM_PRP );
	C_Team *pBrw = GetGlobalTeam( TF_TEAM_BRW );
	...
	SetDialogVariable( "ylwcount", pYlw->Get_Number_Players() );
	SetDialogVariable( "grncount", pGrn->Get_Number_Players() );
	SetDialogVariable( "prpcount", pPrp->Get_Number_Players() );
	SetDialogVariable( "brwcount", pBrw->Get_Number_Players() );
