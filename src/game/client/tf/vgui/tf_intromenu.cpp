		...
		else if ( m_iAction == INTRO_CONTINUE )
		{
			...
			else
			{
					...
					( pPlayer->GetPlayerClass()->GetClassIndex() == TF_CLASS_UNDEFINED ) )
				{
					...
					case TF_TEAM_YLW:
						m_pViewPort->ShowPanel(PANEL_CLASS_YLW, true);
						break;

					case TF_TEAM_GRN:
						m_pViewPort->ShowPanel(PANEL_CLASS_GRN, true);
						break;

					case TF_TEAM_PRP:
						m_pViewPort->ShowPanel(PANEL_CLASS_PRP, true);
						break;

					case TF_TEAM_BRW:
						m_pViewPort->ShowPanel(PANEL_CLASS_BRW, true);
						break;
					}
				}
			}
