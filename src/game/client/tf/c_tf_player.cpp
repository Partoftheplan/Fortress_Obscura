...
#define TF_PLAYER_HEAD_LABEL_YLW 2
#define TF_PLAYER_HEAD_LABEL_GRN 3
#define TF_PLAYER_HEAD_LABEL_PRP 4
#define TF_PLAYER_HEAD_LABEL_BRW 5
...
CLIENTEFFECT_MATERIAL( "models/effects/invulnfx_ylw.vmt" )
CLIENTEFFECT_MATERIAL( "models/effects/invulnfx_grn.vmt" )
CLIENTEFFECT_MATERIAL( "models/effects/invulnfx_prp.vmt" )
CLIENTEFFECT_MATERIAL( "models/effects/invulnfx_brw.vmt" )
...
void C_TFPlayer::InitInvulnerableMaterial( void )
{
	...
	case TF_TEAM_RED:	
		...
		break;
	case TF_TEAM_YLW:
		pszMaterial = "models/effects/invulnfx_ylw.vmt";
		break;
	case TF_TEAM_GRN:
		pszMaterial = "models/effects/invulnfx_grn.vmt";
		break;
	case TF_TEAM_PRP:
		pszMaterial = "models/effects/invulnfx_prp.vmt";
		break;
	case TF_TEAM_BRW:
		pszMaterial = "models/effects/invulnfx_brw.vmt";
		break;
	default:
		break;
	}
...
void C_TFPlayer::OnAddTeleported( void )
{
	if ( !m_pTeleporterEffect )
	{
		...
		case TF_TEAM_RED:
			...
			break;
		case TF_TEAM_YLW:
			pEffect = "player_recent_teleport_ylw";
			break;
		case TF_TEAM_GRN:
			pEffect = "player_recent_teleport_grn";
			break;
		case TF_TEAM_PRP:
			pEffect = "player_recent_teleport_prp";
			break;
		case TF_TEAM_BRW:
			pEffect = "player_recent_teleport_brw";
			break;
		default:
			break;
		}
...
bool C_TFPlayer::IsEnemyPlayer( void )
{
	...
	{
	case TF_TEAM_RED:
		return ( GetTeamNumber() == TF_TEAM_BLUE && TF_TEAM_YLW && TF_TEAM_GRN && TF_TEAM_PRP && TF_TEAM_BRW);
	
	case TF_TEAM_BLUE:
		return ( GetTeamNumber() == TF_TEAM_RED && TF_TEAM_YLW && TF_TEAM_GRN && TF_TEAM_PRP && TF_TEAM_BRW);

	case TF_TEAM_YLW:
		return (GetTeamNumber() == TF_TEAM_RED && TF_TEAM_BLUE && TF_TEAM_GRN && TF_TEAM_PRP && TF_TEAM_BRW);

	case TF_TEAM_GRN:
		return (GetTeamNumber() == TF_TEAM_RED && TF_TEAM_BLUE && TF_TEAM_YLW && TF_TEAM_PRP && TF_TEAM_BRW);

	case TF_TEAM_PRP:
		return (GetTeamNumber() == TF_TEAM_RED && TF_TEAM_BLUE && TF_TEAM_YLW && TF_TEAM_GRN && TF_TEAM_BRW);

	case TF_TEAM_BRW:
		return (GetTeamNumber() == TF_TEAM_RED && TF_TEAM_BLUE && TF_TEAM_YLW && TF_TEAM_GRN && TF_TEAM_PRP);
...
void C_TFPlayer::ShowNemesisIcon( bool bShow )
{
	if ( bShow )
	{
		...
		case TF_TEAM_BLUE:
			...
			break;
		case TF_TEAM_YLW:
			pszEffect = "particle_nemesis_ylw";
			break;
		case TF_TEAM_GRN:
			pszEffect = "particle_nemesis_grn";
			break;
		case TF_TEAM_PRP:
			pszEffect = "particle_nemesis_prp";
			break;
		case TF_TEAM_BRW:
			pszEffect = "particle_nemesis_brw";
			break;
		default:
...
void C_TFPlayer::GetTeamColor( Color &color )
{
	...
	else if (GetTeamNumber() == TF_TEAM_YLW)
	{
		color[0] = 145;
		color[1] = 145;
		color[2] = 55;
	}
	else if (GetTeamNumber() == TF_TEAM_GRN)
	{
		color[0] = 59;
		color[1] = 120;
		color[2] = 55;
	}
	else if (GetTeamNumber() == TF_TEAM_PRP)
	{
		color[0] = 83;
		color[1] = 54;
		color[2] = 95;
	}
	else if (GetTeamNumber() == TF_TEAM_BRW)
	{
		color[0] = 72;
		color[1] = 72;
		color[2] = 46;
...
bool C_TFPlayer::ShouldCollide( int collisionGroup, int contentsMask ) const
{
		...
		collisionGroup == TFCOLLISION_GROUP_ROCKETS )
	{
		...
		case TF_TEAM_YLW:
			if (!(contentsMask & CONTENTS_YLWTEAM))
				return false;
			break;

		case TF_TEAM_GRN:
			if (!(contentsMask & CONTENTS_GRNTEAM))
				return false;
			break;

		case TF_TEAM_PRP:
			if (!(contentsMask & CONTENTS_PRPTEAM))
				return false;
			break;

		case TF_TEAM_BRW:
			if (!(contentsMask & CONTENTS_BRWTEAM))
				return false;
			break;
		}
	}
...
IMaterial *C_TFPlayer::GetHeadLabelMaterial( void )
{
	...
	else
	{
		...
		{
			if (GetTeamNumber() == TF_TEAM_YLW)
			{
				return g_pHeadLabelMaterial[TF_PLAYER_HEAD_LABEL_YLW];
			}
			else
			{
				if (GetTeamNumber() == TF_TEAM_GRN)
				{
					return g_pHeadLabelMaterial[TF_PLAYER_HEAD_LABEL_GRN];
				}
				else
				{
					if (GetTeamNumber() == TF_TEAM_PRP)
					{
						return g_pHeadLabelMaterial[TF_PLAYER_HEAD_LABEL_PRP];
					}
					else
					{
						return g_pHeadLabelMaterial[TF_PLAYER_HEAD_LABEL_BRW];
					}
				}
			}
		}
