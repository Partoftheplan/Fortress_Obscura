...
BEGIN_DATADESC( CTFGameRulesProxy )
	...
	DEFINE_INPUTFUNC(FIELD_FLOAT, "SetGrnTeamRespawnWaveTime", InputSetBlueTeamRespawnWaveTime),
	...
	DEFINE_INPUTFUNC(FIELD_FLOAT, "AddGrnTeamRespawnWaveTime", InputAddBlueTeamRespawnWaveTime),
	...
	DEFINE_INPUTFUNC(FIELD_STRING, "SetGrnTeamGoalString", InputSetBlueTeamGoalString),
	...
	DEFINE_INPUTFUNC(FIELD_INTEGER, "SetGrnTeamRole", InputSetBlueTeamRole),
END_DATADESC()
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameRulesProxy::InputSetGrnTeamRespawnWaveTime(inputdata_t &inputdata)
{
	TFGameRules()->SetTeamRespawnWaveTime(TF_TEAM_GRN, inputdata.value.Float());
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameRulesProxy::InputAddGrnTeamRespawnWaveTime(inputdata_t &inputdata)
{
	TFGameRules()->AddTeamRespawnWaveTime(TF_TEAM_GRN, inputdata.value.Float());
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameRulesProxy::InputSetGrnTeamGoalString(inputdata_t &inputdata)
{
	TFGameRules()->SetTeamGoalString(TF_TEAM_GRN, inputdata.value.String());
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTFGameRulesProxy::InputSetGrnTeamRole(inputdata_t &inputdata)
{
	CTFTeam *pTeam = TFTeamMgr()->GetTeam(TF_TEAM_GRN);
	if (pTeam)
	{
		pTeam->SetRole(inputdata.value.Int());
	}
...
void CTFGameRules::SetTeamGoalString( int iTeam, const char *pszGoal )
{
	...
	else if (iTeam == TF_TEAM_GRN)
	{
		if (!pszGoal || !pszGoal[0])
		{
			m_pszTeamGoalStringGrn.GetForModify()[0] = '\0';
		}
		else
		{
			if (Q_stricmp(m_pszTeamGoalStringGrn.Get(), pszGoal))
			{
				Q_strncpy(m_pszTeamGoalStringGrn.GetForModify(), pszGoal, MAX_TEAMGOAL_STRING);
			}
		}
	}
}
...
*/
	...
	bool CTFGameRules::CheckWinLimit()
	{
		if ( mp_winlimit.GetInt() != 0 )
		{
			...
			else if (TFTeamMgr()->GetTeam(TF_TEAM_GRN)->GetScore() >= mp_winlimit.GetInt())
			{
				UTIL_LogPrintf("Team \"GRN\" triggered \"Intermission_Win_Limit\"\n");
				bWinner = true;
			}
...
void cc_ShowRespawnTimes()
{
	...
	if ( pRules && pPlayer )
	{
		...
		float flGrnMin = (pRules->m_TeamRespawnWaveTimes[TF_TEAM_GRN] >= 0 ? pRules->m_TeamRespawnWaveTimes[TF_TEAM_GRN] : mp_respawnwavetime.GetFloat());
		float flGrnScalar = pRules->GetRespawnTimeScalar(TF_TEAM_GRN);
		float flNextGrnRespawn = pRules->GetNextRespawnWave(TF_TEAM_GRN, NULL) - gpGlobals->curtime;
		...
		char tempGrn[128];
		Q_snprintf(tempGrn, sizeof(tempGrn), "Grn: Min Spawn %2.2f, Scalar %2.2f, Next Spawn In: %.2f\n", flGrnMin, flGrnScalar, flNextGrnRespawn);

		ClientPrint( pPlayer, HUD_PRINTTALK, tempRed );
		ClientPrint( pPlayer, HUD_PRINTTALK, tempBlue );
		ClientPrint(pPlayer, HUD_PRINTTALK, tempGrn);
	}
}
...
void CTFGameRules::SendWinPanelInfo( void )
{
	...
	if ( winEvent )
	{
		...
		int iGrnScore = GetGlobalTeam(TF_TEAM_GRN)->GetScore();
		int iBlueScorePrev = iBlueScore;
		int iRedScorePrev = iRedScore;
		int iGrnScorePrev = iGrnScore;
		...
		if ( bRoundComplete && !bScoringPerCapture )
		{
			...
			case TF_TEAM_RED:
				...
				break;
			case TF_TEAM_GRN:
				iGrnScorePrev = (iGrnScore - TEAMPLAY_ROUND_WIN_SCORE >= 0) ? (iGrnScore - TEAMPLAY_ROUND_WIN_SCORE) : 0;
				break;
...
void CTFGameRules::FillOutTeamplayRoundWinEvent( IGameEvent *event )
{
	...
	{
	case TF_TEAM_RED:
		iLosingTeam = TF_TEAM_BLUE && TF_TEAM_GRN;
		break;
	case TF_TEAM_BLUE:
		iLosingTeam = TF_TEAM_RED && TF_TEAM_GRN;
		break;
	case TF_TEAM_GRN:
...
void CTFGameRules::SetupSpawnPointsForRound( void )
{
	...
	while( pSpot )
	{
		...
		if ( pTFSpawn )
		{
			...
			CHandle<CTeamControlPointRound> hRoundGrn = pTFSpawn->GetRoundGrnSpawn();
...
const char *CTFGameRules::GetTeamGoalString( int iTeam )
{
	...
	if (iTeam == TF_TEAM_GRN)
		return m_pszTeamGoalStringGrn.Get();
	return NULL;
}
