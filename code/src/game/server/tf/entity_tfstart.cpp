...
BEGIN_DATADESC( CTFTeamSpawn )
	...
	DEFINE_KEYFIELD(m_iszRoundGrnSpawn, FIELD_STRING, "round_grnspawn"),
...
void CTFTeamSpawn::InputRoundSpawn( inputdata_t &input )
{
	...
	if (m_iszRoundGrnSpawn != NULL_STRING)
	{
		// We need to re-find our control point round, because they're recreated over round restarts
		m_hRoundGrnSpawn = dynamic_cast<CTeamControlPointRound*>(gEntList.FindEntityByName(NULL, m_iszRoundGrnSpawn));
		if (!m_hRoundGrnSpawn)
		{
			Warning("%s failed to find control point round named '%s'\n", GetClassname(), STRING(m_iszRoundGrnSpawn));
		}
	}
}
