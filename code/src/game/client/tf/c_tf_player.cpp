...
bool C_TFPlayer::IsEnemyPlayer( void )
{
	...
	{
	case TF_TEAM_RED:
		return ( GetTeamNumber() == TF_TEAM_BLUE && TF_TEAM_GRN);
	
	case TF_TEAM_BLUE:
		return ( GetTeamNumber() == TF_TEAM_RED && TF_TEAM_GRN);

	case TF_TEAM_GRN:
		return (GetTeamNumber() == TF_TEAM_RED && TF_TEAM_BLUE);
...
bool C_TFPlayer::ShouldCollide( int collisionGroup, int contentsMask ) const
{
		...
		collisionGroup == TFCOLLISION_GROUP_ROCKETS )
	{
		...
		case TF_TEAM_GRN:
			if (!(contentsMask & CONTENTS_UNUSED6))
				return false;
			break;
		}
	}
