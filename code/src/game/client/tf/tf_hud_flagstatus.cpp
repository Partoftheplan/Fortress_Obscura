...
void CTFArrowPanel::Paint()
{
	...
	else if (pEnt->GetTeamNumber() == TF_TEAM_GRN)
	{
		pMaterial = m_GrnMaterial;
		...
		if (pLocalPlayer && (pLocalPlayer->GetObserverMode() == OBS_MODE_IN_EYE))
		{
			...
			if (pTargetEnt && pTargetEnt->IsPlayer())
			{
				...
				if (pTarget->HasTheFlag() && (pTarget->GetItem() == pEnt))
				{
					pMaterial = m_GrnMaterialNoArrow;
				}
			}
		}
	}
...
void CTFHudFlagObjectives::OnTick()
{
	...
	for ( int i = 0; i < g_Flags.Count(); i++ )
	{
		...
		if ( pFlag )
		{
			if ( !pFlag->IsDisabled() )
			{
				...
				else if (m_pGrnFlag && pFlag->GetTeamNumber() == TF_TEAM_GRN)
				{
					m_pGrnFlag->SetEntity(pFlag);
				}
			}
		}
		else
	...
	if ( tf_flag_caps_per_round.GetInt() > 0 )
	{
		...
		pTeam = GetGlobalTFTeam(TF_TEAM_GRN);
		if (pTeam)
		{
			SetDialogVariable("grnscore", pTeam->GetFlagCaptures());
		}
	...
	else // we're just playing straight score
	{
		...
		if (pTeam)
		{
			SetDialogVariable("grnscore", pTeam->Get_Score());
		}
